#프로젝트4주차 
# 아래 코드에서 사용될 모든 변수와 패키지를 선언해줍니다. 
# 전체 코드 시스템은 함수들로 구성된 시스템입니다. 
# 변경하고 싶은 변수는 현재 셀에서 변경하면 유용합니다 ! 
...
import numpy as np 
import sqlite3
from scipy.optimize import minimize #최소값을 가지게 하는 변수의 해를 구하는 모듈  
...
smtp_info = dict({"smtp_server" : "smtp.naver.com", # SMTP 서버 주소
                  "smtp_user_id" : "sanguktemp7664@naver.com", #개인 이메일주소 xxx@naver.com
                  "smtp_user_pw" : "tkddnrqnrP12" , #naver 비밀번호
                  "smtp_port" : 587}) # SMTP 서버 포트

#사용자가 누가될 지 모르니 아래와 같이 아래 모든 코드의 결과물에 적용될 root를 설정하여 수정에 용이하게 합니다.
path = r"C:\Users\sangu\Desktop\4주차첨부자료"
...
def DB_parsing_for_1year():
    ...
    datelist = datelist_oneyear()
    res = res[res['DATE'].isin(datelist)]
...
def get_weights_for_lowvol(cov): 
    cons = ({'type': 'eq', 'fun': lambda w: np.sum(w) - 1}) #weight의 합은 1의 조건 추가 
    bnds = [(0,1)]*len(cov) #각 성분들의 바운더리를 설정합니다. (공매도 허용시 [-1,1])
    w = np.ones(len(cov))/len(cov) # weight의 초기값 
...
def send_email(date =datetime.datetime.strftime(today, '%Y%m%d') ):
    ...
    multi['to'] = "sanguktemp7664@naver.com"
